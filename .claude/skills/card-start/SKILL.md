...
4. カードに紐づくPRを**すべて**特定する
   ...
   - **PRのURLが1件も見つからない場合だけ**、カード名 `[B7] ...` → ブランチ `feature/fire-fire-b7` → `gh pr list --head feature/fire-fire-b7 --state all --json number,state,mergedAt,url` で引く(フォールバック)
...
5. **フォールバックで見つけたPRは、カードより後にマージされたものだけ採る**(コメント由来のPRには要らない)
   - カードの作成日時は**カードIDから導ける**。TrelloのIDはMongoDBのObjectIdで、先頭8桁(16進)がepoch秒
     ...
     ```
   - `mergedAt` がこれより**前**のPRは別カードのものなので**捨てる**。捨てた結果0件になったらフォールバックは失敗
...
**フォールバックのマージ日時のガードを省かない。** カードIDは再利用される。`[X0-8]` は「HTMLモックのパレットを揃える」カードで一度使われ、そのブランチ `feature/fire-fire-x0-8` とPR #79 が残ったまま、後日まったく別の「Skillsの精査」で同じIDが振られた。カード名からブランチ名を導くフォールバックは、この2枚を区別できない。**PRを1本も出していない新しいカードが、別カードのマージ済みPRを理由に完了へ動く。**

実際の値で確かめられる: PR #79 のマージは `2026-08-08T12:08:19Z`、新しい `[X0-8]`(ID `6a7751a96c0eebb2a2d97faa`)の作成は `2026-08-08T15:56:25Z`。マージのほうが**前**なので捨てられる。
