...
**head が `release/*` だったときは、ここでブランチを削除する**(正本 10章 手順5)。リポジトリの `delete_branch_on_merge` は無効なので、マージしただけでは消えない。消し忘れると、次のリリースで切り直したものと並んでどれが出したものか分からなくなる。

```bash
gh api -X DELETE repos/KunitoObara/FIRE_FIRE/git/refs/heads/release/YYYY-MM-DD
```

**削除はデプロイの成功を確かめたあとに行う。** 失敗していたときに、何を出したのかを追える状態を残しておく(内容は `main` に入っているので復元はできるが、探す手間が変わる)。
