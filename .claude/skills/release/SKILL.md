...
- **「基底が古い」は起きない。** `main` の up-to-date チェックは有効なままだが(正本 10章「`main` の『基底が古い』は実害が無い」)、`main` が進むのはリリースPRのマージ時だけで、リリースPRを2本同時に開かない運用のため、このPRが開いている間に `main` が進むことはない。もし出たら、その前提(2本目のリリースPRが動いている、`main` に直接コミットが入った、等)が崩れているということなので、`develop` へ `main` を取り込む前にPOへ報告する
...
**head が `release/*` だったときは、報告に「リリースブランチの削除」を含める**(正本 10章 手順5)。リポジトリの `delete_branch_on_merge` は無効なので、マージしただけでは消えない。残ったまま次のリリースを切ると、どれが出したものか分からなくなる。

**削除するのはPO。** マージ済みPRに出る Delete branch ボタン、または `git push origin --delete release/YYYY-MM-DD`。リモートのブランチを消すのはこの工程の受け持ちではなく、`gh api` での書き込みは遮断フックが拒否する。`/release` がするのは**促すところまで**。

**促すのはデプロイの成功を確かめたあと。** 失敗していたときに、何を出したのかを追える状態を残しておく(内容は `main` に入っているので復元はできるが、探す手間が変わる)。
