...
1. **利ざやの符号**。ローン残高 > 時価 の物件で負の値が返り、そのまま表示されるか。`Math.max(0, ...)` や `Math.abs()` で潰していないか
   ...
   - **B1の円グラフでも落とさない。** 資産種別には「0円以下を除く」フィルタが掛かっているが、不動産・負債の擬似分類には掛けない(出さないのは合計がちょうど0円のときだけで、負なら絶対値で面積を取り凡例に符号付きで出す)。既存のフィルタを擬似分類にもそのまま適用すると、オーバーローンの分類軸だけ不動産が円グラフから消える([B4-8-1](https://trello.com/c/qkSOISE4))
