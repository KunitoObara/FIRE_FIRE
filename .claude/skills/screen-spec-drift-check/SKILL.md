...
description: Cross-checks implemented frontend screens/pages against the screen requirements docs (docs/screen-list-and-transitions.md and docs/screen-requirements-*.md) — screen IDs A1-A10 and B1-B11 (A0 and B12-B17 are specified but unimplemented, and are not reported as drift), their listed display/input fields, and their transition conditions. Use this skill after implementing or modifying a page/route under src/frontend, or when the user asks whether a screen "matches the spec" or is "done" — proactively run it once real page components exist, since drift is easy to miss without an explicit diff against the docs.
---
...
docs/screen-list-and-transitions.md と各 docs/screen-requirements-*.md は、画面ID(A0・A1〜A10, B1〜B17。うち実装済みは A1〜A10(A0を除く)と B1〜B11)ごとに表示項目・入力項目・主な操作・遷移条件を細かく定義している。画面実装が積み上がるにつれて、要件定義書の更新漏れや実装側の解釈違いによる乖離が起きやすい。このスキルは実装後に定期的に差分を検出するためのものであり、実装前の設計フェーズでは単に要件定義書を読めばよい。
...
| 画面ID | ルート | コンポーネント |
|---|---|---|
| A9 利用規約 | `(public)/terms` | `components/public/` |
...
### A0 と B12〜B17 は表に無い(未実装)

**A0 サービストップページとPhase 5〜7の画面(B12〜B17)は、ルートもコンポーネントもまだ存在しない。** 要件だけが先に決まっている状態で、A0は docs/screen-requirements-public.md に、B12〜B14は docs/screen-requirements-lists.md に、B15〜B17は docs/screen-list-and-transitions.md 2.8に概要だけがある。

**A9・A10は実装済みで、上の表に入っている**([A0](https://trello.com/c/BT4yT3Zk) の1本目のPRで公開用シェルとともに入った)。この2画面は対象であり、要件との乖離は報告する。
...
- **A0が未実装であることに由来する状態は差分として報告しない。** 具体的には、`/` が `(setup-check)` のプレースホルダーを返すこと、A9・A10のヘッダーのロゴのリンク先(`/`)がまだA0でないこと、公開用シェルにヒーロー等のA0固有のセクションが無いこと
...
2. 該当する要件定義書のセクションを読む。
   ...
   - 公開画面(A0・A9・A10。**フェーズ外。A9・A10は実装済み、A0は未実装**) → docs/screen-requirements-public.md
