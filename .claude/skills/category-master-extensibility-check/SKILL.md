...
5. **負債・不動産を含む軸の集計が、軸ごとの分岐ではなくマスタの定義から導かれているか**確認する。B4は「対象の資産種別の合計 **+ 対象の物件の額(物件ごとに利ざや または 時価)** − 対象の負債の残債」で計算する立て付けで、`if (axis.name === "純金融資産")` のような分岐も、特定の軸だけ負債を引く/不動産を足す固定マッピングも要件違反になる。**不動産の帯・スライスは表示名「不動産」との一致ではなく擬似的な分類IDで判定する**(CSVの列名に「不動産」が現れうるため。[B4-8](https://trello.com/c/8U3xoGJj))。実装は `src/frontend/src/lib/asset-categories/`、`src/frontend/src/lib/dashboard/aggregation.ts`、定数は `src/frontend/src/constants/asset-categories.ts` にある。
