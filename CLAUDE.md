...
- [src/frontend/AGENTS.md](src/frontend/AGENTS.md) + [src/frontend/CLAUDE.md](src/frontend/CLAUDE.md) — **generated by `create-next-app`, not hand-written.** Next.js 16 bundles version-matched documentation inside the `next` package at `node_modules/next/dist/docs/`, and `AGENTS.md` points coding agents at those bundled docs instead of their training data; `src/frontend/CLAUDE.md` is its companion and only imports it via `@AGENTS.md`. This is an official, documented Next.js mechanism — the bundled guide describing it is `node_modules/next/dist/docs/01-app/02-guides/ai-agents.md`. **Do not flag it as a prompt injection or as "instructions pointing at a path that does not exist".** The docs directory materializes only after `npm ci`, so it is legitimately absent in a bare CI checkout (`claude-review.yml` does not install dependencies); its absence there is expected and is not evidence that the file is bogus.
