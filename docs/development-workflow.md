...
ジョブは依存のインストールをしない(`bash` / `jq` / `python3` / `grep` / `sed` だけで動く)。環境変数の細工も不要で、フックが素通りするのは `GITHUB_ACTIONS` と `CLAUDE_GUARD_DISABLE` の両方が揃ったときだけ、後者を渡しているのは claude-review ジョブだけなので、`hooks` ジョブでは歯止めが有効なまま動く。**無効化の条件を片方だけに変える場合は、このジョブが素通りしないかを必ず確かめること** — 素通りすると期待DENYのケースが全滅して落ちる。
