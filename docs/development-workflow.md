...
**理由はこれだけである。** 「`main` の『基底が古い』を解くため」は理由にしない — その状態自体を無くしてある(次項)。
...
### `main` の「基底が古い」を無くしてある

**`main` では「Require branches to be up to date before merging」を有効にしない**([ci-cd-setup.md](./ci-cd-setup.md) 6章)。`develop` では有効のままにする。

リリースPRをマージすると、そのマージコミットは `main` にだけ残り `develop` へは戻さない。**つまり `develop` は必ず `main` の先端を含まない状態になり、リリースのたびに再発する**(この判断をした 2026-08-16 時点で17件)。有効のままだと、直PRを出すたびに `main` を `develop` へ取り込むことになり、STGへ出したものと `develop` の先端がずれる。リリースブランチで迂回することもできるが、それでは「大きい回だけの例外」が毎回必要になり、例外ではなくなる。

**外して失うものは小さい。** このチェックが見ているのは「PRを作ったあとに base が進み、その組み合わせでは壊れる」場合だが、`main` が進むのはリリースのマージのときだけで、その中身は必ず `develop` を通ってSTGで動いたものである。リリースPRを2本同時に開かない規定(手順の要点6、`/release` の判定表)もあるため、base が進んでいる最中のリリースPRという状態自体が起きない。**CI4ジョブの必須は外さない。**
