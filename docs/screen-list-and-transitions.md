...
- **現在の破線は、共通ヘッダー/サイドバーから B12〜B14(2.7)への3本である。** Phase 5の3画面はまだ実装されていない。この規約は[B6-2](https://trello.com/c/amf5APQu)で先に決めたもので、当時は適用先が無く規約だけが残っていた
...
- **サイドバーに並ぶ画面が増え続けたときの整理**。Phase 5でB12〜B14が入ると11画面が縦に並ぶ。グループ見出しを入れるか折りたたむかは、実際に並べてから決める(先に決めても、どの並びが読みにくいかは並べるまで分からない)
