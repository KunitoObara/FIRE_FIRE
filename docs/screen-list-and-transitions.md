...
| (フェーズ外) | A0・A9・A10・A11(2.0) | **4画面とも実装済み。** 詳細要件を [screen-requirements-public.md](./screen-requirements-public.md) に持つ。フェーズの段に乗らない理由は [要件定義書 7.3](./fire-asset-management-requirements.md#73-サービスの公開面をフェーズ順に乗せない理由) |
...
| 画面ID | 画面名 | 概要 | 関連要件 |
|---|---|---|---|
| A0 | サービストップページ | 未ログインの訪問者に本サービスの内容を伝えるランディングページ。ヘッダーからA1・A4へ送り出し、フッターからA9・A10・A11へ導く。**ベータ版であることを明記する**。**実装済み**(`/`) | 要件定義書 4.15 |
...
**A1〜A8と分けてあるのは、認証フローに属さないためである。** A1〜A8も未ログインで到達するが、あちらは「アカウントを作る/ログインする」手順の途中にあり、行き着く先はB1である。この4画面はどのフローにも属さず、訪問者はそのまま帰ってよい。ログイン中に開いてもB1へリダイレクトしない点も、A1〜A8(ログイン済みならガードで送り出される)と違う。
...
**A9〜A11の画面IDが末尾の番号になっているのは、後から追加された経緯によるものである**(2.2のB11と同じ)。位置付けはA0と同じ公開画面であり、A8の次に続く認証画面ではない。
...
- **現在の破線は、共通ヘッダー/サイドバーから B12〜B14(2.7)への3本だけである。** 公開画面(2.0)は4画面とも実装済みになったため、出入りする矢印はすべて実線に戻してある。この規約は[B6-2](https://trello.com/c/amf5APQu)で先に決めたもので、当時は適用先が無く規約だけが残っていた
...
### 3.4 公開画面の遷移(A0・A9・A10・A11)

2.0の4画面を抜き出したもの。**4画面とも実装済みのため、すべて実線である**(3.0)。合流先(A1 / A4 / B1)は3.1・3.3と同一の画面である。
...
>
> **A9・A10のフッターにA0への導線は置かない。** フッターに並ぶのは利用規約・プライバシーポリシー・お問い合わせの3本だけで、A0へ戻れるのはヘッダーのロゴからである(図のラベルもそうしてある)。規約を読んでいる人がフッターまで到達したときに欲しいのは「もう一方の規約」であって、トップページではない。
>
> **フッターは4画面共通のため、A9・A10・A11の間は互いに行き来できる。** 図が描いているのはA9⇄A10の2本だけで、他の組み合わせも同じフッターから同じように辿れる(全通り引くと本数のわりに読み取れることが増えない)。
>
