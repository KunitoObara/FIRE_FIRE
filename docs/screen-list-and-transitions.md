...
| (フェーズ外) | A0・A9・A10・A11・A12(2.0) | **5画面とも実装済み。** 詳細要件を [screen-requirements-public.md](./screen-requirements-public.md) に持つ。フェーズの段に乗らない理由は [要件定義書 7.3](./fire-asset-management-requirements.md#73-サービスの公開面をフェーズ順に乗せない理由) |
...
| 画面ID | 画面名 | 概要 | 関連要件 |
|---|---|---|---|
| A0 | サービストップページ | 未ログインの訪問者に本サービスの内容を伝えるランディングページ。ヘッダーからA1・A4へ送り出し、フッターからA9・A10・A11・A12へ導く。**ベータ版であることを明記する**。**実装済み**(`/`) | 要件定義書 4.15 |
| A9 | 利用規約画面 | 利用条件の掲示。A0のフッターとA1の同意チェックからリンクする。**実装済み**(`/terms`) | 要件定義書 4.15 |
| A10 | プライバシーポリシー画面 | 取得する情報とその扱いの掲示。リンク元はA9と同じ。問い合わせ先としてA11へリンクする。**実装済み**(`/privacy`) | 要件定義書 4.15 |
| A11 | お問い合わせ画面 | 運営者への問い合わせフォーム。A0のフッターとA10の本文からリンクする。**A0には入力項目を置かないため独立した画面にしてある**。**実装済み**(`/contact`) | 要件定義書 4.15 |
| A12 | ヘルプページ | 使い方ガイド・よくある質問・用語集を掲示する。リンク元はA9・A10と同じくA0のフッター。**A0の実装時点では未着手で見送られていたもの**([X2](https://trello.com/c/tgP5d1Ue))。**実装済み**(`/help`) | 要件定義書 4.15 |

**A1〜A8と分けてあるのは、認証フローに属さないためである。** A1〜A8も未ログインで到達するが、あちらは「アカウントを作る/ログインする」手順の途中にあり、行き着く先はB1である。この5画面はどのフローにも属さず、訪問者はそのまま帰ってよい。ログイン中に開いてもB1へリダイレクトしない点も、A1〜A8(ログイン済みならガードで送り出される)と違う。
...
**A9〜A12の画面IDが末尾の番号になっているのは、後から追加された経緯によるものである**(2.2のB11と同じ)。位置付けはA0と同じ公開画面であり、A8の次に続く認証画面ではない。
...
### 2.6 画面IDを持たない共通の画面・操作

特定の画面に属さず、複数の画面に共通して現れるもの。**独立した画面IDは設けない。**
...
| 画面 | 出る条件 | 表示 | 関連要件 |
|---|---|---|---|
| エラー画面(`src/frontend/src/app/global-error.tsx`) | ルートレイアウトごと巻き込んだ描画エラーが起きたとき。**公開画面(2.0)・認証系(2.1)・ログイン後(2.2〜2.5)のどこでも出る** | 見出し「エラーが発生しました」、一文の説明、「再読み込み」ボタンのみ | 要件定義書 5章(可観測性) |

**画面IDを振らないのは、どのフローにも属さないためである。** A系(公開・認証)にもB系(ログイン後)にも同じように出るので、既存の分類のどこに入れても定義がぶれる。A0〜A12・B1〜B17のどれとも並列ではなく、それらの上に重なって出るものとして扱う([X3-2](https://trello.com/c/IIOZKmNl))。

**この画面はアプリ側の共通コンポーネント・共通レイアウトを使わない。意図した制約である。** ここが動くのは既に何かが壊れているときなので、共通ボタンやレイアウトを経由すると、その経路自体が壊れている場合に画面ごと出せなくなる。**見た目を[DESIGN.md](../DESIGN.md)の体裁へ寄せるほど、この「出せない」リスクは上がる。** 素の要素とTailwindのクラスだけで組む現状は、体裁を整えきれていないのではなく、そこに寄せない判断の結果である。同じ理由で**A11 お問い合わせへの導線も置いていない**。

**セグメント単位の `error.tsx` は置かない。** 画面ごとの部分的な復帰(ダッシュボードだけ再試行する等)はできないが、`global-error.tsx`が受け止めてSentryへ送るため**検知に穴は無い**。画面ごとに文言と復帰方法を決める作業に見合う場面が出てから足す。

**遷移図(3章)には描かない。** どの画面からでも出るため、描くと図中のほぼ全ノードから矢印を引くことになり、利用者の意図した遷移が読めなくなる。B15〜B17を載せない理由(3.0)とは別で、あちらは行き先が未定だからである。
...
節番号が2.6の共通の画面・操作より後になっているのは、既存の節番号を動かさないためである(2.6は本ドキュメントの他の箇所・[DESIGN.md](../DESIGN.md)・HTMLモックから参照されている)。並びの意図ではない。
...
- **現在の破線は、共通ヘッダー/サイドバーから B12〜B14(2.7)への3本だけである。** 公開画面(2.0)は5画面とも実装済みになったため、出入りする矢印はすべて実線に戻してある。この規約は[B6-2](https://trello.com/c/amf5APQu)で先に決めたもので、当時は適用先が無く規約だけが残っていた
...
> **A0からの2本も実線である**(3.0。A0は実装済みで `/` が応答する)。A0を経由せず直接A1・A4を開く経路(`Start` からの実線2本)も無くならない — ブックマークやログアウト後の遷移先はA4のままである。
>
...
flowchart TD
    ...
    AccountSettings -->|アカウント削除| Top[A0 サービストップページ]
```
...
> **B10からのアカウント削除はシェルの外へ出る。** 削除するとセッションが無効になるため、成功時は A0 サービストップページへ履歴を置き換えて遷移し、完了メッセージをA0で1回だけ出す([auth-login-requirements.md](./auth-login-requirements.md) 3.11、[screen-requirements-account.md](./screen-requirements-account.md)「アカウントの削除」)。ログアウト(A4へ)と行き先が違うのは、削除した人はもう利用者ではないためである。
...
### 3.4 公開画面の遷移(A0・A9・A10・A11・A12)

2.0の5画面を抜き出したもの。**5画面とも実装済みのため、すべて実線である**(3.0)。合流先(A1 / A4 / B1)は3.1・3.3と同一の画面である。
...
flowchart TD
    ...
    Top -->|フッター:ヘルプ| Help[A12 ヘルプページ]
    Top -->|フッター:お問い合わせ| Contact[A11 お問い合わせ画面]
...
>
> **A1からA9・A10への2本は、同意チェック内のリンクである。** A9・A10の実装でリンク先を差し替えたため実線になった。**この2本だけは別タブで開く** — 同意する前に本文を読むためのリンクで、同じタブで遷移すると入力済みのメールアドレスとパスワードが消えるため。
>
> A9・A10からA1・A4へは引いていない。規約を読み終えた人が向かう先は元いた画面であり、ヘッダーのロゴでA0へ戻る導線とブラウザの戻る操作で足りる。
>
> **A9・A10のフッターにA0への導線は置かない。** フッターに並ぶのは利用規約・プライバシーポリシー・ヘルプ・お問い合わせの4本だけで、A0へ戻れるのはヘッダーのロゴからである(図のラベルもそうしてある)。規約を読んでいる人がフッターまで到達したときに欲しいのは「もう一方の規約」であって、トップページではない。
>
> **フッターは5画面共通のため、A9・A10・A11・A12の間は互いに行き来できる。** 図が描いているのはA9⇄A10の2本だけで、他の組み合わせも同じフッターから同じように辿れる(全通り引くと本数のわりに読み取れることが増えない)。
>
> **A11から出る矢印はヘッダーのロゴの1本だけである。** 送信できても画面を遷移させない([screen-requirements-public.md](./screen-requirements-public.md) A11) — 未ログインの利用者に次の行き先が無く、結果はその場で伝える。
>
> **A12から出る矢印もヘッダーのロゴの1本だけである。** A9・A10と同じ静的な内容の画面で、送信のような操作を持たないため([screen-requirements-public.md](./screen-requirements-public.md) A12)。
