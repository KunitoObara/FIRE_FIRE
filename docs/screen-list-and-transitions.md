...
| (フェーズ外) | A0・A9・A10(2.0) | **A9・A10は実装済み、A0は未実装。** 詳細要件を [screen-requirements-public.md](./screen-requirements-public.md) に持つ。フェーズの段に乗らない理由は [要件定義書 7.3](./fire-asset-management-requirements.md#73-サービスの公開面をフェーズ順に乗せない理由) |
...
- **現在の破線は、共通ヘッダー/サイドバーから B12〜B14(2.7)への3本と、A0 サービストップページに出入りする分(3.1・3.4)である。** どちらもまだ実装されていない。**A9・A10は実装済みのため実線に戻してある**(A0との間の矢印だけがA0側の未実装で破線のまま)。この規約は[B6-2](https://trello.com/c/amf5APQu)で先に決めたもので、当時は適用先が無く規約だけが残っていた
...
2.0の3画面を抜き出したもの。**A9・A10は実装済みで、未実装のA0に出入りする矢印だけが破線である**(3.0)。合流先(A1 / A4 / B1)は3.1・3.3と同一の画面である。
