...
| (フェーズ外) | A0・A9・A10(2.0) | **3画面とも実装済み。** 詳細要件を [screen-requirements-public.md](./screen-requirements-public.md) に持つ。フェーズの段に乗らない理由は [要件定義書 7.3](./fire-asset-management-requirements.md#73-サービスの公開面をフェーズ順に乗せない理由) |
...
| 画面ID | 画面名 | 概要 | 関連要件 |
|---|---|---|---|
| A0 | サービストップページ | 未ログインの訪問者に本サービスの内容を伝えるランディングページ。ヘッダーからA1・A4へ送り出し、フッターからA9・A10へ導く。**ベータ版であることを明記する**。**実装済み**(`/`) | 要件定義書 4.15 |
| A9 | 利用規約画面 | 利用条件の掲示。A0のフッターとA1の同意チェックからリンクする。**実装済み**(`/terms`) | 要件定義書 4.15 |
| A10 | プライバシーポリシー画面 | 取得する情報とその扱いの掲示。リンク元はA9と同じ。**実装済み**(`/privacy`) | 要件定義書 4.15 |
...
- **現在の破線は、共通ヘッダー/サイドバーから B12〜B14(2.7)への3本だけである。** 公開画面(2.0)は3画面とも実装済みになったため、出入りする矢印はすべて実線に戻してある。この規約は[B6-2](https://trello.com/c/amf5APQu)で先に決めたもので、当時は適用先が無く規約だけが残っていた
...
2.0の3画面を抜き出したもの。**3画面とも実装済みのため、すべて実線である**(3.0)。合流先(A1 / A4 / B1)は3.1・3.3と同一の画面である。
...
>
> **A9・A10のフッターにA0への導線は置かない。** フッターに並ぶのは利用規約・プライバシーポリシーの2本だけで、A0へ戻れるのはヘッダーのロゴからである(図のラベルもそうしてある)。規約を読んでいる人がフッターまで到達したときに欲しいのは「もう一方の規約」であって、トップページではない。
