 ...
 * 書けばフィールドごと消える — これが「1分待たせない」の実体である。**そのため`lastSentAt`を
 * 持たない記録が残りうる。** 将来この一覧をTTLで掃除するなら、期限フィールドは`lastSentAt`
 * ではなく`windowStartedAt`にすること — TTLは対象フィールドを持たないドキュメントを削除せず、
 * この関数を通った記録だけが永久に残る(A11「スパム対策」)。`windowStartedAt`なら、書くか
 * ドキュメントごと消すかのどちらかなので、生き残る記録は必ず持っている。
 *
