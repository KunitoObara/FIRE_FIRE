 ...
 *
 * **日付・時刻の末尾に`(?!\d)`が要る。** 選択肢は各開始位置で「最初に成功した分岐」で
 * 確定し、より長く一致する分岐を探しに戻らない。これが無いと`1000-2000`が
 * 日付として`1000-20`で確定し、**金額がまるごと素通りする**(残りの`00`は2桁なので
 * `\d{4,}`にも掛からない)。`Content-Range`の`bytes 1000-2000/5000`のような
 * 外部由来の文字列でも同じことが起きる。時刻側も同様で、`01:2345`が
 * `01:23`で確定して残りが読める形で残ってしまう。
 *
...
const NUMERIC_PATTERN =
  /(\d{4}-\d{2}(?:-\d{2})?(?!\d)|\d{2}:\d{2}(?::\d{2})?(?!\d))|\d{1,3}(?:,\d{3})+|\d{4,}/g;
