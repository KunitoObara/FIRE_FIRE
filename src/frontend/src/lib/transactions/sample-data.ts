 ...
 * ある。データの繋ぎ込み時にこのファイルごと削除し、
 * `src/lib/transactions/transactions-data.ts`の取得元を差し替える。
 *
 * 基準日を引数で受け、同じ基準日なら常に同じ値を返す(乱数を使わない。描画のたびに数値が
 * 変わるとサーバーとクライアントで表示がずれるため)。
 */
 ...
 * 同じ組み合わせなら常に同じ値になるようにしている。1〜26日に収め、月末日数の差(28〜31日)を
 * 気にしなくて済むようにする。
 */
