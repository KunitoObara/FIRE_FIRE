  ...
  <LegalDocument title="プライバシーポリシー">
    ...
    <ol>
      ...
      */}
      ...
      {/*
        アカウントの削除はB10 アカウント設定から実行できる([X7])。パスワードでのログインが
        無いアカウント(Googleのみ)は本人確認を通せないため画面からは実行できず、その場合の
        受け口は問い合わせになる(docs/auth-login-requirements.md 3.11)。
      */}
      <li>
        登録データの削除と、アカウントそのもの(メールアドレス・認証情報を含む)の削除は別のお求めです。
        <strong>
          アカウントの削除は、ログイン後のアカウント設定画面からご自身で実行できます。
        </strong>
        削除するとデータは復元できません。Googleのみでログインしているアカウントは画面から削除できないため、8のお問い合わせ先までご連絡ください。
      </li>
    </ol>
