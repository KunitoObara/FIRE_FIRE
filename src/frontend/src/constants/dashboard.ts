...
/**
 * 分類別内訳の凡例に添えるリスクレベルの読み上げ用ラベル
 * (docs/screen-requirements-dashboard.md B1「リスクの可視化」)。
 *
 * 画面には出さない(`sr-only`)。凡例の行は「分類名 リスク 構成比 金額」と読み上げられるが、
 * 「低」「中」「高」の1文字だけでは何の段階なのかが音声では分からないため、前に付けて
 * 何を指しているかを補う。見た目の側は形状(○ / △ / ◇)がそれを担う。
 */
export const RISK_LEVEL_LEGEND_LABEL = "リスク";
