 ...
 * 比較も文字列の辞書順なので保存まで素通りする(`2020-13` < 当月)。B11の入力欄は
 * `MonthPicker`([Y-01](https://trello.com/c/SQRBn52d)でネイティブの`<input type="month">`から
 * 置き換えた)で、値は`buildMonthKey`が1〜12の固定リストから組み立てるため、**画面からは
 * 不正な月を作れない**。それでもこの形を保つのは、同じパターンがFirestoreから読んだ
 * ドキュメントの検査(`debtDocumentSchema`)にも使われるためで、画面を経ずに書かれた値が
 * そのままグラフの起点になるのを止める。
 */
